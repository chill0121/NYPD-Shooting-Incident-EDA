...
```
Looks like there are quite a few blanks and entries labeled as "unknown" some features have both. 
We'll combine these by labeling all blanks as unknown. This data likely comes from officers on the scene and either
missed some information or, in the case of the perp, might not have had a witness or not caught the offender. 
...
for(i in 1:length(import_url)){
    # If the column is a factor AND/OR contains empty space or 'U'
...
Instead of assuming what they intended we will change these to 'UNKNOWN'.
