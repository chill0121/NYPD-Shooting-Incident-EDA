...
Let's take a look at the dimension of this imported data.frame as well as the variable types of each column and summary.
```{r EDA}
dim(import_url)
str(import_url)
summary(import_url)
...
Looks like we have 19 columns (features) and 25596 rows (datapoints).  
First, let's remove any features that we won't be needing for our analysis. 
- `JURISDICTION_CODE` is pretty broad for localizing shooting incidents so we will end up using `BORO` instead which will give more insight to our analysis.
- `X_COORD_CD`, `Y_COORD_CD`, and `Lon_Lat` are all redundant.
...
import_url(head)
```
Next, we will check if there are any missing values or duplicated, focusing only on the `INCIDENT_KEY` feature for now.
...
For better analysis that will be upcoming we should change the class type
of a few of these features

## Utilize Coordinate Data to Visualize Superimposed Data on Map
Here we're going to visualize the location of each shooting incident using the coordinates given in the dataset. 
First, we can use the minimum and maximum values of each coordinate to find the map's bounding box 
...
superimpose our data on that same coordinate system.
```{r Map Visualization}
