...
For reference here is a map of NYC's boroughs taken from public domain on Wikipedia: <https://upload.wikimedia.org/wikipedia/commons/thumb/3/34/5_Boroughs_Labels_New_York_City_Map.svg/1920px-5_Boroughs_Labels_New_York_City_Map.svg.png>
```{r, Reference Map, out.width = '400px', fig.align = 'center'}
reference_map <- 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/34/5_Boroughs_Labels_New_York_City_Map.svg/1920px-5_Boroughs_Labels_New_York_City_Map.svg.png'
...
incident_map_point
```
This looks great and gives a much deeper understanding of the data, much further at least than the previous borough plot.
...
- Central Park seems to be quite safe from shootings.
- The North side of Manhattan is responsible for the majority of the shootings there.
- The distribution of shootings in Queens is quite spread out and mostly on the South side.
- There's some hot spots that were not obvious without the coordinate data, namely around the coasts.

Let's try to convert this same data into a heat/density map to get a better picture of these higher density areas.
