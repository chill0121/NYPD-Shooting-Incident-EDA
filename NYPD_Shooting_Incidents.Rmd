...
We'll combine these by labeling all blanks as "UNKNOWN". This data likely comes from officers on the scene and either
...
- The remaining features are missing a few, but is not a significant amount compared to the size of the dataset.
...
Here, we cannot assume what they intended so we will change these age values to 'UNKNOWN'.
...
# Plot graph
import_url %>%
   ggplot(., aes(x = reorder(BOROUGH, BOROUGH, length, decreasing = TRUE), fill = BOROUGH)) + geom_bar(aes(y = (..count..))) +
...
incident_map_point <- ggmap(get_stamenmap(map_bounds, maptype = 'terrain', zoom = 11)) + 
        ...
        labs(x = 'LONGITUDE', y = 'LATITUDE', caption = 'Source:<https://catalog.data.gov/dataset/nypd-shooting-incident-data-historic>')
# Display Point Map
incident_map_point
